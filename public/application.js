...
var checkForWin = function checkForWin() {
  ...
  if (currentPlayer === tiles[0] && currentPlayer === tiles[1] && currentPlayer === tiles[2]) {
    ...
    $('.gameboard').hide(800);
    $('#game-message').html('');
  } else if (tiles.every(function (value, index, array) {
    return value !== '';
  })) {
    store.gameData.game.over = true;
...
var onUpdateGame = function onUpdateGame(event) {
  var dataId = $(event.target).data('id'); // assigns a variable to data-id
  ...
  api.updateGame().then(ui.updateGameSuccess).then(function () {
    checkForWin();
    playerSwitch();
